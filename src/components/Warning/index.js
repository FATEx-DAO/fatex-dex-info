...
export default function Warning({ type, show, setShow, address }) {
  ...
  const textContent = below800 ? (
    <div>
      ...
      <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
        Similar to the Harmony Explorer, this site automatically tracks analytics for all HRC20 tokens independent of
        token integrity. Please do your own research before interacting with any HRC20 token.
      </Text>
    </div>
  ) : (
    <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
      Anyone can create and name any HRC20 token on Harmony, including creating fake versions of existing tokens and
      tokens that claim to represent projects that do not have a token. Similar to the Harmony Explorer, this site
      automatically tracks analytics for all HRC20 tokens independent of token integrity. Please do your own research
